...
export const EyeSlashFilledIcon: React.FC<SvgIconProps> = (props) => (
  <svg
    aria-hidden="true"
    fill="currentColor"
    focusable="false"
...
export const EyeFilledIcon: React.FC<SvgIconProps> = (props) => (
  <svg
    aria-hidden="true"
    fill="currentColor"
    focusable="false"
...
const StyledInput: React.FC<InputConfig> = (props) => {
  const [isVisible, setIsVisible] = React.useState(false);
  const toggleVisibility = () => setIsVisible(!isVisible);

  // If the type is password, render the input with the toggle button
  if (props.type === 'password') {
    return (
      <Input
        // The common 'email-basic' style is defined here
        className="max-w-xs"
        // Pass down all props from the child (label, placeholder, etc.)
        {...props}
        // Dynamically change the type for visibility
        type={isVisible ? "text" : "password"}
        // Add the icon button ONLY for password fields
        endContent={
          <button
  ...
  // NEW: Render a search input with an icon
  if (props.type === 'search') {
    return (
        <Input
            // Common 'email-basic' style
            className="max-w-xs"
            {...props}
            startContent={
  ...
  // For all other types (e.g., 'email', 'text'), render a standard input
  // with the same common style but without the icon.
  return (
    <Input
      className="max-w-xs"
      {...props}
...
/**
 * The main parent component that receives an array of input configurations
 * and renders a StyledInput for each one.
 */
interface InputTypeComponentsProps {
  inputConfigs: InputConfig[];
}

const InputTypeComponents: React.FC<InputTypeComponentsProps> = ({ inputConfigs }) => {
  return (
...
export default InputTypeComponents;